...
const OPTIONS = ['TikTok', 'YouTube', 'Friend', 'App Store', 'Other'];

export default function Question8() {
  const router = useRouter();
  const [selectedIndex, setSelectedIndex] = React.useState<number | null>(null);

  // bigPad = xxl if available, else 2x xl
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      ...
      >
        {/* Header with extra bottom spacing (2.5x) */}
      ...
      >
        <Pressable
          ...
          onPress={() => router.push('/(public)/agree')}
          style={[
            styles.cta,
            {
              backgroundColor: theme.colors.text, // enabled color via token
              opacity: canContinue ? 1 : 0.45, // disabled state
            },
          ]}
          accessibilityRole="button"
